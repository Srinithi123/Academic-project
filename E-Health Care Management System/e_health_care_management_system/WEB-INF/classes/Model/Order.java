...
public class Order extends Connect
{
	...
	public String saveOrder(HashMap orderData)
	{
		...
		try
		{
            ...
            while(rs.next())
			{
				///// Update the Doctor Stock //////
				SQL = "UPDATE doctor SET doctor_specialization = (doctor_specialization - "+rs.getString("cart_quantity")+") WHERE doctor_id = "+rs.getString("cart_doctor_id");
				pstmt = connection.prepareStatement(SQL);
				pstmt.executeUpdate();
				
				///// Insert Into Order Items //////
				SQL = "INSERT INTO `order_item` (`oi_order_id`, `oi_doctor_id`, `oi_price_per_unit`, `oi_cart_quantity`, `oi_total`) VALUES (?, ?, ?, ?, ?);";
				pstmt = connection.prepareStatement(SQL);
				pstmt.setString(1, String.valueOf(last_inserted_id));
				pstmt.setString(2, rs.getString("cart_doctor_id"));
    ...
    public ArrayList getAllOrder(int userID, int vendor_id)
	{
		...
		if(userID != 0)
			...
			SQL = "SELECT * FROM `order`,`order_item`, `order_status`,`doctor` WHERE doctor_id = oi_doctor_id AND order_id = oi_order_id AND order_status = os_id AND doctor_email = "+vendor_id+" GROUP BY oi_order_id";
		else
    ...
    public ArrayList getAllOrderItems(String orderID)
	{
		int count=0;
		String SQL = "";
		SQL = "SELECT * FROM `order_item`,`doctor` WHERE oi_doctor_id = doctor_id AND oi_order_id = "+orderID;
        ArrayList resultArray = new ArrayList();
        try
		{
			statement = connection.createStatement();
			...
			{		
				HashMap results = new HashMap();
				results.put("oi_doctor_id",rs.getString("oi_doctor_id"));
