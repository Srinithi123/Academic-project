...
public class Stock extends Connect
{
	...
	{
		String SQL = "INSERT INTO stock (stock_doctor_id, stock_number) VALUES (?, ?);";
		int record=0; 
		String error = "";
		...
		try
		{
			...
			pstmt.setString(1,(String) stockData.get("stock_doctor_id"));
    ...
    public HashMap getStockDetails(int stock_id)
	{
		...
		try
		{
            String SQL =  "SELECT * FROM `stock`,`doctor` WHERE stock_id = "+stock_id ;
            statement = connection.createStatement();
            rs = statement.executeQuery(SQL);
            while(rs.next())
			{
				results.put("stock_doctor_id",Integer.parseInt(rs.getString("stock_doctor_id")));
			...
			if(count==0)
			{
				results.put("stock_doctor_id",0);
				results.put("stock_number","");
				results.put("doctor_name","");
				results.put("doctor_image","no-image.jpg");	
	...
	{
		String SQL = "UPDATE stock SET stock_doctor_id = ?, stock_number = ? WHERE stock_id = ?;";
		String error = "";
    ...
    public ArrayList getAllStock(int doctor_id)
	{
		String SQL =  "SELECT * FROM `stock`,`doctor` WHERE doctor_id = stock_doctor_id and doctor_id = "+doctor_id ;
		int count=0;
	...
	public String getDoctorOption(Integer SelID)
    {
		int selectedID = SelID.intValue();
    	return Connect.getOptionList("doctor","doctor_id","doctor_id","doctor_id,doctor_id",selectedID,"1");
    }
}
