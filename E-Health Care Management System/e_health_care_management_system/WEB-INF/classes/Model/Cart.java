...
public class Cart extends Connect
{
	...
	public String saveCart(HashMap cartData)
	{
		...
		try
		{
			SQL = "SELECT * FROM `cart` WHERE cart_doctor_id = "+cartData.get("cart_doctor_id");
			...
			if(count != 0) {
				SQL="DELETE FROM cart WHERE cart_doctor_id = "+cartData.get("cart_doctor_id");
				statement.executeUpdate(SQL);
			}
			
			//// Insert Item into cart ////
			SQL = "INSERT INTO `cart` (`cart_user_id`, `cart_doctor_id`, `cart_quantity`, `cart_total`, `cart_price_per_unit`) VALUES (?, ?, ?, ?, ?);";
			int record=0; 
		
			pstmt = connection.prepareStatement(SQL);
			pstmt.setString(1,(String) cartData.get("cart_user_id"));
			pstmt.setString(2,(String) cartData.get("cart_doctor_id"));
	...
	{
		String SQL = "SELECT * FROM `cart`,`doctor` WHERE doctor_id = cart_doctor_id";
		int count=0;
        ...
        try
		{
			...
			{		
				...
				results.put("cart_doctor_id",Integer.parseInt(rs.getString("cart_doctor_id")));
